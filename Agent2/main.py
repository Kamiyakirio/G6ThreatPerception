...
def create_asset_detect_message_callback(mac_address):
    def callback(ch, method, properties, body):
        ...
        if data['info']['type'] == 'assets':
            ...
            routing_key = "detect_result"
        elif data['info']['type'] == 'apprisk':
            detect_result = app_risk_detect(data)
            routing_key = "apprisk_detect_result"
        else:
            print(f"未知检测类型: {data['info']['type']}")
            routing_key = None

        if detect_result and routing_key:
            producer = RabbitProducer(
                host=HOST, port=PORT, username=USERNAME, password=PASSWORD
            )
            producer.publish_message("", routing_key, detect_result)
